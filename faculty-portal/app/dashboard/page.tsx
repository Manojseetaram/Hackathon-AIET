...
//         loadDashboardData(parsedFaculty.id)
//       }
//     }, [])
...
//     Welcome Prof. {faculty?.name?.split(" ").slice(-1)[0] || ""} 👋
//   </h1>
//   <p className="text-muted-foreground mt-1">Here's an overview of your teaching activities</p>
// </div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (facultyData) {
      const parsedFaculty: Faculty = JSON.parse(facultyData)
      setFaculty(parsedFaculty)
      loadDashboardData(parsedFaculty.id) // ✅ use correct field name
    }
  }, [])
  ...
  try {
    ...
    const subjectsResponse = await fetch(
      `http://localhost:8080/subjects/faculty?faculty_id=${facultyId}`,
      {
        headers: {
    ...
    const normalizedSubjects: Subject[] = Array.isArray(subjectsData)
      ? subjectsData
      : subjectsData?.data || []
    ...
    for (const subject of normalizedSubjects) {
      const studentsResponse = await fetch(
        `http://localhost:8080/attendance/summary/subject/${subject.subject_code}`, // ✅ use subject_code in path
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (Array.isArray(studentsData)) {
        ...
        allSummaries.push(...studentsData.data)
      }
    }
  ...
  const averageAttendance =
    studentSummary.length > 0
      ? Math.round(
          studentSummary.reduce((sum, student) => sum + student.Percentage, 0) /
