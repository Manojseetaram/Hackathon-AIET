...
export default function AttendanceHistoryPage() {
  ...
  const API_BASE = "http://localhost:8080" // Replace with your actual backend base URL
  ...
  // Direct API call to fetch subjects assigned to faculty
  const loadSubjects = async (facultyData: Faculty) => {
    try {
      const res = await fetch(`${API_BASE}/faculty/${facultyData.faculty_id}/subjects`)
  ...
  // Direct API call to fetch attendance by subject and date
  const handleSearchHistory = async () => {
    if (!selectedSubject || !selectedDate) {
      toast({
        title: "Missing Information",
    ...
    try {
      ...
      const data: AttendanceWithNames[] = await res.json()
      ...
      toast({
        title: "Error",
        description: error.message || "Error fetching attendance history",
        variant: "destructive",
      })
      setAttendanceHistory([])
      setHistoryLoaded(true)
